...
class BluetoothChatService(
    ...
    fun connect(device: BluetoothDevice, secure: Boolean) {
        if (connectionPool[device.address] == null) {
            bluetoothConnection = BluetoothConnection(this, device)
            connectionPool[device.address] = bluetoothConnection!!
        }
        connect(connectionPool[device.address]!!, secure)
    }

    @Synchronized
    fun connect(bluetoothConnection: BluetoothConnection, secure: Boolean) {
        val device = bluetoothConnection.bluetoothDevice
        Log.d(TAG, "connect to: $device")
        if ((this.bluetoothConnection == bluetoothConnection) and (state == STATE_CONNECTED)) {
            return
        }
    ...
    private inner class ConnectedThread(socket: BluetoothSocket?, socketType: String) :
        ...
        init {
            ...
            try {
                ...
                val bc = connectionPool[socket.remoteDevice.address]!!
                if (bc.devPtr == 0L) {
                    bc.devPtr = addToRadar(radar.radarPtr, bc)
                }
            } catch (e: IOException) {
        ...
        override fun run() {
            ...
            while (this@BluetoothChatService.state == STATE_CONNECTED) {
                try {
                    ...
                    bluetoothConnection!!.inputStream = ByteArrayInputStream(buffer, 0, bytes)
